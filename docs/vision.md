...
| K11 | **Repo-aware context.** In a git repo, load `CLAUDE.md`, `AGENTS.md`, `.agentic.toml` from the repo root into orchestrator context automatically, tagged as project instructions and subject to the taint rules (a repo file can *inform*, never *authorise*). Small enough to ship in Phase 1. | S | A2, I1 |
| K12 | **Session sharing.** `agentic share [--approve-only] [--ttl 1h]` prints a one-time link or tmux socket path; a colleague gets a read-only view of the session, or an approve-only view that can answer INBOX items but not type commands. Every remote approval is logged with who granted it. | M | G1, E6, I4 |
