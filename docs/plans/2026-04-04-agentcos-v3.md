...
if __name__ == "__main__":
    ...
    from shell.telemetry.db import DB_PATH
    config_path = _Path.home() / ".config" / "sable" / "config.json"
...
```bash
rtk git add .
rtk git commit -m "feat: Sable v3 task engine and adaptive skill learning complete"
```
