...
2. Read `docs/roadmap-phases.md` for what is being built and in what order, and `docs/structure.md` for where code belongs.
3. Pick a phase gate or a feature ID (`A1`, `B2`, … from `docs/vision.md`) and open an issue naming it before starting large work.
...
- Every user-visible behaviour must be observable: an event on the bus, a block on screen, or a `/why`-style command. See `docs/structure.md` §4.
